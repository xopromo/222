...
# In-memory store: state -> {code_verifier, device_id}
...
class VKOAuthService:
    ...
    async def exchange_code(self, code: str, state: Optional[str] = None, device_id: Optional[str] = None) -> dict:
        session = _session_store.pop(state, {}) if state else {}
        code_verifier = session.get("code_verifier")
        saved_device_id = session.get("device_id") or device_id
        ...
        if saved_device_id:
            data["device_id"] = saved_device_id
